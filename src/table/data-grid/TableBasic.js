...
const ClickableEmailCell = ({ email }) => {
  const handleEmailClick = () => {
    window.location.href = `mailto:${email}`;
  };

  return (
    <a href={`mailto:${email}`} onClick={handleEmailClick} style={{ cursor: 'pointer', color: 'black' }}>
      {email}
    </a>
  );
};
...
const initialColumns = (handleEditClick) => [
  {
    flex: 0.1,
    field: 'id',
    minWidth: 80,
  ...
  },
  {
    ...
    headerName: 'Email',
    editable: true,
    renderCell: (params) => <ClickableEmailCell email={params.value} />
  },
  {
    flex: 0.15,
    minWidth: 120,
...
const initialRows = [
  ...
  const TableBasic = () => {
    ...
    const handleEditClick = (row) => {
      const newName = prompt("Enter a new name:", row.full_name);
      if (newName) {
        const updatedRow = { ...row, full_name: newName };
        const updatedRows = rows.map((r) => (r.id === row.id ? updatedRow : r));
        setRows(updatedRows);
        setFilteredRows(updatedRows);
        handleSaveClick(updatedRows);
      }
    };
    ...
    const handleSaveClick = (updatedRows) => {
    ...
    const columns = initialColumns(handleEditClick);
      ...
      <Card>
        ...
        <div style={{ height: 350, width: 'auto' }}>
          <input
            ...
            style={{ marginBottom: '10px', padding: '10px', width: '200px', border: '1px solid black' }}
          />
